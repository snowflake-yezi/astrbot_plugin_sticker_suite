...
class StickerSuitePlugin(Star):
    ...
    STICKER_TTL_SECONDS = STICKER_TTL_SECONDS
    CONTEXT_WINDOW_SECONDS = CONTEXT_WINDOW_SECONDS
    RECENT_TEXT_LIMIT = RECENT_TEXT_LIMIT
    ...
    def __init__(self, context: Context):
        ...
        self.pending_tag_tasks: dict[str, asyncio.Task] = {}
    ...
    def _sticker_line(self, index: int, key: str, sticker: dict[str, Any]) -> str:
        return sticker_line(
            ...
            key,
            sticker,
            ensure_metadata=self._ensure_sticker_metadata,
            short_id=self._short_id,
            source_has_identity=self._source_has_identity,
        )
    ...
    def _find_sticker_index(self, group: dict[str, Any], shared: dict[str, Any], key: str, sticker: dict[str, Any]) -> int | None:
        return find_sticker_index(self._indexed_stickers_with_shared(group, shared), key, sticker)

    def _sticker_detail_text(self, group: dict[str, Any], shared: dict[str, Any], key: str, sticker: dict[str, Any]) -> str:
        return sticker_detail_text(
            group,
            shared,
            key,
            sticker,
            ...
            short_id=self._short_id,
            source_has_identity=self._source_has_identity,
            indexed_stickers_with_shared=self._indexed_stickers_with_shared,
        )
